...
{
  ros::init(argc, argv, "circle_publish_node"); // ROS 노드를 초기화하고 노드 이름을 "basic_publish_node"로 설정
  ros::NodeHandle n; // ROS 노드 핸들러 생성

  geometry_msgs::Twist cmd_vel; // geometry_msgs 패키지의 Twist 메시지 타입을 사용하여 cmd_vel 객체 생성
  ros::Publisher publisher_cmd_vel = n.advertise<geometry_msgs::Twist>("/turtle1/cmd_vel", 1000); // cmd_vel 토픽에 Twist 메시지를 발행하는 Publisher 생성

  ros::Rate loop_rate(60); // 주기를 60Hz로 설정하는 Rate 객체 생성

  while (ros::ok()) // ROS가 정상적으로 실행 중인 동안 반복
  {
    cmd_vel.linear.x = 2.0; // 선속도(x축 방향)
    cmd_vel.angular.z = 1.0; // 각속도(z축 방향)
